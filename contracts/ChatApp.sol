...
contract ChatApp{
    ...
    mapping (bytes => message[]) allMessages;
    ...
    function addFriend(address friend_key, string calldata name)external{
        ...
        require(checkAlreadyFriends(msg.sender,friend_key)==false,"These are already friends");
...
function sendMessage(address friend_key, string calldata _msg) external {
    ...
    bytes memory myBytes = abi.encodePacked(chatCode);

    message memory newMsg = message(msg.sender,block.timestamp , _msg);

    allMessages[myBytes].push(newMsg);
}
